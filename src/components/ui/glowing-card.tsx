...
export const GlowingCard: React.FC<GlowingCardProps> = ({
  ...
  return (
    <div
      className={cn(
        "relative flex-1 min-w-[14rem] rounded-2xl",
  ...
  return (
    <div className={cn("relative w-full", className)} style={containerStyle}>
      ...
      >
        <div
          className={cn(
            "flex items-start justify-center flex-wrap gap-[var(--gap)]",
        ...
        {enableGlow && (
          ...
          >
            <div
              className={cn(
                "flex items-start justify-center flex-wrap gap-[var(--gap)] max-w-[var(--max-width)] mx-auto",
                responsive && "flex-col sm:flex-row"
              )}
              style={{ padding: "var(--padding)" }}
            >
              {React.Children.map(children, (child) => {
                if (React.isValidElement(child) && child.type === GlowingCard) {
                  const cardGlowColor = child.props.glowColor || "#fb923c";

...
>(({ className, children, forceMount, ...props }, ref) => {
  ...
  const { open } = context;
  const contentRef = React.useRef<HTMLDivElement>(null); // Ref to measure content height // Combine external ref with internal ref
    ...
    <AnimatePresence initial={false}>
      ...
      {(open || forceMount) && (
        <motion.div
          ...
          style={{ overflow: "hidden" }} // Ensure content is clipped during animation // The onAnimationStart and onAnimationComplete are not strictly necessary for basic height animation, // but can be useful for more complex scenarios or debugging. // onAnimationStart={() => { /* maybe add data-state="animating" */ }} // onAnimationComplete={() => { /* maybe remove data-state="animating" */ }}
          className={cn(
